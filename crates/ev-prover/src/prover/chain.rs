...
impl ChainContext {
    ...
    pub async fn from_config(config: Config, ism_client: Arc<CelestiaIsmClient>) -> Result<Arc<Self>> {
        ...
        let celestia_client = Arc::new(Client::new(&config.rpc.celestia_rpc, auth_token, None, None).await?);
        let evm_provider =
    ...
    pub async fn celestia_ws_client(&self) -> Result<Client> {
        ...
        Client::new(url.as_str(), auth_token, None, None).await.map_err(|e| anyhow!(e))
    }
